...
static void dump_rom(FF_IOMAN *ioman)
{
    ...
    print_line(COLOR_CYAN, 1, "   Dumping some shit...");
...
static void  __attribute__((long_call)) (*sf_command_sio)(uint32_t command[], void * out_buffer, int out_buffer_size, int toggle_cs);
static void  __attribute__((long_call)) (*srom_init_maybe)(void) = 0;
static void sf_read(uint32_t addr, uint8_t * buf, int size)
{
    ...
    for (int i = 0; i < size; i++)
    {
        if (i % COUNT(teaspoon) == 0)
        {
            ...
            if (i % (COUNT(teaspoon)*256)  == 0) printf("\b\b\b\b\b\b\b\b%08X",i);
        }
...
static void sf_dump(int drive)
{
  ...
  switch (get_model_id()) {
    case 0x350 : //"80D"
      SF_SIZE = 0x800000;
      sf_command_sio = (void*)0x10456C;
      srom_init_maybe = (void *) 0x105710;
      break;
    case 0x393 : // "750D"
      SF_SIZE = 0x800000;
      sf_command_sio = (void*)0x104DA8;
      srom_init_maybe = (void *)0x00105f4c;
      break;
    default :
  ...
  /* initialize buffer */
  uint8_t (*buffer) = (uint8_t *)0x42000000;
  ...
  *(volatile uint32_t *)0xD2090008 |= 0x200000;
  printf(" - ROM Init\n");
  srom_init_maybe();
  // now read from address addr into buffer
  int addr = 0x00;
  sf_read(addr,buffer,SF_SIZE);
  ...
  //for (uint32_t i=0; i<SF_SIZE;i+=0x10) {
  //uint32_t i=0;
  //      Add   00   01   02   03   04   05   06   07   08   09   0a   0b   0c   0d   0e  0f
  //printf("%X  %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\n"
  //   ,buffer+i,(char)buffer[i],(char)buffer[i+1],(char)buffer[i+2],(char)buffer[i+3]
  //   ,(char)buffer[i+4],(char)buffer[i+5],(char)buffer[i+6],(char)buffer[i+7]
  //   ,(char)buffer[i+8],(char)buffer[i+9],(char)buffer[i+10],(char)buffer[i+11]
  //   ,(char)buffer[i+12],(char)buffer[i+13],(char)buffer[i+14],(char)buffer[i+15]);
  //}
...
static void dump_rom_with_canon_routines()
{
    if (is_digic6())
    {
        printf(" - Dumping my shit dont mind me...\n");
...
static void init_canon_routines() {
...
cstart( void )
{
    ...
    void *meme = 0x40106200;
    goto *meme;
    print_line(COLOR_CYAN, 3, "  Magic Lantern Parra\n");
    ...
    #if ((defined(CONFIG_BOOT_DUMPER) && !defined(CONFIG_BOOT_FULLFAT) && !defined(CONFIG_BOOT_DISP_DUMP)) || defined(CONFIG_BOOT_SROM_DUMPER))
        /* we need to initialize the canon routines */
        init_canon_routines();
    #endif
